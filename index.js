...
ws.on("open", function open() {
  ...
  const subscriptions = [
    ...
    { type: "set_coin", coinSymbol: config.coin.toLowerCase() },
    { type: "set_coin", coinSymbol: config.coin },
  ];
  ...
  console.log(`\nMonitoriting ${config.coin} price and transactions...\n`);
});
...
ws.on("message", function message(data) {
  ...
  try {
    ...
    const messageHashRaw = MessageObject.data ? JSON.stringify(MessageObject.data) : Date.now().toString()
    ...
    if (!HashMap.has(messageHash)) {
      ...
      if (MessageObject.type === "ping") {
        ...
        if (TransactionObject.coinSymbol == config.coin && config.webhooks.transactions) {
          if (ACTION == "BOUGHT") {
            FireWebhook(
              config.webhooks.transactions,
              `\`${TransactionObject.username}\` bought \`${TransactionObject.amount}\` ${config.coin} for $${TransactionObject.totalValue}`,
              {
                color: "green",
            ...
            FireWebhook(
              config.webhooks.transactions,
              `\`${TransactionObject.username}\` sold \`${TransactionObject.amount}\` ${config.coin} for $${TransactionObject.totalValue}`,
              {
                color: "red",
        ...
        if (MessageObject.coinSymbol == config.coin && config.webhooks.value) {
          console.log(
